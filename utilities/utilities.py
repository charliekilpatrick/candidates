...
def search_legacy_files(coord, radius=30 * u.arcsec,
    base_dir='/data2/ckilpatrick/photoz/legacy/'):
    ra = coord.ra.degree
    dec = coord.dec.degree
    ...
    for r in ra_lim:
        ...
        for d in dec_lim:
            ...
            if file not in files:
                files.append(file)
                table = Table.read(base_dir+file, hdu=1)
                mask = ((table['RA'] < r_limits[1]) &
                    (table['RA'] > r_limits[0]) &
                    (table['DEC'] > d_limits[0]) &
                    (table['DEC'] < d_limits[1]))
                pzfile = file.replace('.fits','-pz.fits')
                pztable = Table.read(base_dir+pzfile, hdu=1)
...
def get_ps1strm(coord, radius=300 * u.arcsec, **kwargs):
    ...
    catalog = '/data2/ckilpatrick/photoz/gw190814.cat'
    file = 'tmp'

    cmd = 'cat {0} | '.format(catalog)
    cmd += 'awk -F\',\' \'{if (($3 '
    cmd += '{0} )^2+($4 '.format(ra)
    cmd += '{0} )^2 <'.format(dec)
...
def check_asassn(coord, *args, **kwargs):
    # Max radius is 10.0
    radius = 10.0
...
def format_redshift(row, **kwargs):
    ...
    if row[use_name]!='--':
        ...
        if use=='legacy': source='LDR8'
        if use=='2mpz': source='2MPZ'
...
def download_yse(yse):
    ...
    for key in table.keys():
        if 'transient_ra' in key: table.rename_column(key, 'ra')
        if 'transient_dec' in key: table.rename_column(key, 'dec')
        if 'disc_date' in key: table.rename_column(key, 'discovery_date')
        if 'spec_class' in key: table.rename_column(key, 'classification')
...
def check_class(table, **kwargs):
    ...
    classtable = ascii.read(kwargs['classification'])

    classification = []
    for row in table:
        match = classtable['name']==row['name']
        if len(classtable[match])>0:
            classification.append(classtable[match][0]['class'])
        else:
    ...
    class_col = Column(['--' not in str(row['classification'])
...
def add_data(table, proc, **kwargs):
    ...
    if procname not in table.keys() or redo:
        ...
        for i,row in enumerate(table):
            ...
            if best_match:
                b=best_match
                coldata.append([True,str(b['name']).strip(),b['ra'],b['dec']])
            else:
...
def add_phot_data(table, photdata, **kwargs):
